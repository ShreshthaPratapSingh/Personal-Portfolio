...
getLoginClick.addEventListener("click", async function () {
    ...
    if (getLoginTEXT && getLoginPasskey) {
        const response = await fetch("https://portfolio-backend-ta0x.onrender.com/verify-login", {
            method: "POST",
        ...
        if (result.success) {
            ...
            const notiData = await fetch("https://portfolio-backend-ta0x.onrender.com/Notifications-Hiring", { method: "GET" });
